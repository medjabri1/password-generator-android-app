...
public class MainActivity extends AppCompatActivity {
    ...
    public void generate(View view) {
        ...
        if(!check_capital_letters.isChecked() && !check_small_letters.isChecked() && !check_numbers.isChecked() && !check_symbols.isChecked()) {
            //No choices selected
            Toast.makeText(this, "Select one item at least", Toast.LENGTH_LONG).show();
            return;
        }

        if(et_password_length.length() < 1) {
            et_password_length.setText(String.valueOf(MIN_LENGTH));
            Toast.makeText(this, "Length should not be empty", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        if(current_val < MIN_LENGTH) {
            et_password_length.setText(String.valueOf(MIN_LENGTH));
            Toast.makeText(CONTEXT, "Min value is "+ MIN_LENGTH, Toast.LENGTH_LONG).show();
            return;
        }

        else if(current_val > MAX_LENGTH) {
            et_password_length.setText(String.valueOf(MAX_LENGTH));
            Toast.makeText(CONTEXT, "Max value is "+ MAX_LENGTH, Toast.LENGTH_LONG).show();
            return;
        }
    ...
    public void copy(View view) {
        ...
        ClipData clip = ClipData.newPlainText("Random password", tv_password.getText().toString());
        Objects.requireNonNull(clipboard).setPrimaryClip(clip);
        Toast.makeText(this, "Random password copied", Toast.LENGTH_LONG).show();
    }
}
